...
const goTolocalScore = (total) => {
  let OldScore = Number(localStorage.getItem("scoreQuizMainPhp"));
  if (total > OldScore) {
    localStorage.setItem("scoreQuizMainPhp", total);
    OldScore = total;
  }
  return OldScore;
};
...
//Codigo para obtener las preguntas desde la base de datos

const getQuestions = async () => {
  try {
    const response = await fetch("../php/getQuestions.php");
